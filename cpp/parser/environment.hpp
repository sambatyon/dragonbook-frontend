...
inline std::shared_ptr<Environment> Environment::create(std::shared_ptr<Environment> previous) {
  return std::make_shared<Environment>(previous);
}

inline Environment::Environment(std::shared_ptr<Environment> previous) : previous_(previous), table_() {
}

inline void Environment::put(std::shared_ptr<lexer::Token> token, std::shared_ptr<inter::Identifier> id) {
  table_.insert(std::make_pair(token, id));
}

inline std::shared_ptr<inter::Identifier> Environment::get(std::shared_ptr<lexer::Token> token) const {
