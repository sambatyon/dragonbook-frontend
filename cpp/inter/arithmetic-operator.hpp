...
class Arithmetic : public Operator {
 public:
  ...
  Arithmetic(std::shared_ptr<lexer::Token> token, std::shared_ptr<Expression> expr1, std::shared_ptr<Expression> expr2);
  virtual ~Arithmetic();
...
inline std::shared_ptr<Arithmetic> Arithmetic::create(
    std::shared_ptr<lexer::Token> token,
    std::shared_ptr<Expression> expr1,
    std::shared_ptr<Expression> expr2
) {
...
inline std::shared_ptr<Expression> Arithmetic::expr1() const {
  return expr1_;
}

inline std::shared_ptr<Expression> Arithmetic::expr2() const {
  return expr2_;
}
