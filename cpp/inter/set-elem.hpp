...
inline std::shared_ptr<SetElem> SetElem::create(std::shared_ptr<Access> access, std::shared_ptr<Expression> expr) {
  return std::make_shared<SetElem>(access, expr);
}

inline SetElem::SetElem(std::shared_ptr<Access> access, std::shared_ptr<Expression> expr)
...
inline std::shared_ptr<Identifier> SetElem::array() const {
  return array_;
}

inline std::shared_ptr<Expression> SetElem::index() const {
  return index_;
}

inline std::shared_ptr<Expression> SetElem::expr() const {
  return expr_;
}
}  // namespace inter
