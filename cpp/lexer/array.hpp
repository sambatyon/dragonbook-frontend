...
inline std::shared_ptr<Array> Array::create(const std::size_t &element_count, std::shared_ptr<Type> type) {
  return std::make_shared<Array>(element_count, type);
}

inline Array::Array(const std::size_t &element_count, std::shared_ptr<Type> type)
...
inline std::shared_ptr<Type> Array::type() const {
  return type_;
}

inline std::size_t Array::element_count() const {
