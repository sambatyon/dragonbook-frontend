...
inline
std::shared_ptr<Word> Word::create(const std::string &lexeme, const std::uint32_t &tag) {
    return std::make_shared<Word>(lexeme, tag);
}

inline
Word::Word(const std::string &lexeme, const std::uint32_t &tag) : Token(tag), lexeme_(lexeme) {
}
